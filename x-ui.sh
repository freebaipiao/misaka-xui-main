...
install() {
    bash <(curl -Ls https://raw.githubusercontents.com/freebaipiao/misaka-xui-main/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $yn =~ "Y"|"y" ]]; then
        ...
        last_version=$(curl -Ls "https://api.github.com/repos/freebaipiao/misaka-xui-main/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ -z "$last_version" ]]; then
            red "检测 x-ui 版本失败，可能是超出 Github API 限制，正在使用备用源检测最新版本"
            last_version=$(curl -sm8 https://raw.githubusercontents.com/freebaipiao/misaka-xui-main/main/config/version)
            if [[ -z "$last_version" ]]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/freebaipiao/misaka-xui-main/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        wget -N --no-check-certificate https://raw.githubusercontents.com/freebaipiao/misaka-xui-main/main/x-ui.sh -O /usr/bin/x-ui
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/freebaipiao/misaka-xui-main/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
